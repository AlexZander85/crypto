...
AI-разбор материала урока: только текст текущего урока, без прогнозов и рекомендаций. Реальная модель живёт на воркере Workers AI (опционален); фронт работает и без ключа — через мок-режим  для самопроверки.

- **F1 Проверка Фейнмана**: объяснение ученика → JSON-вердикт understood|partial|missed → cn_feynman_ai
- **F2 Объясни иначе**: 3 переупаковки (Проще/Пример/Короче) одного уровня
- **F3 Сократов**: 3 ступени встречных вопросов → итог из материала
- **F4 Генератор задач**: модель оформляет условие, КОД считает эталон (плечо, Келли, Sharpe)
- **H1 «Подскажи»** в каждом виджете: 3 ступени, счётчик cn_hints ≤2 для бейджа мастерства
...
- Лимит 30 действий/день (пул по всем 7 фичам); выходной фильтр «цена вырастет/покупай»
- Гейт: cn_tier_override/__TIER__ = 'max'; апселл-модалка для нижних тарифов
