// ===== Этап 5, регрессия: старый ридер, Этапы 1–4 не сломаны, смоки lp5:*, скриншоты =====
...
(async () => {
  ...
  await page.screenshot({ path: path.join(SHOT_DIR, 'shot5_reader_dash.png') });
  ...
  // ==== 4) Этап 3: хаб (теперь с «Недавними тестами») ====
  ...
  ok('4. Этап 3: хаб открывается; секция «Недавние тесты» Этапа 5 на месте', e3.hub && e3.controls > 5 && e3.recentTests, JSON.stringify(e3));
  await page.screenshot({ path: path.join(SHOT_DIR, 'shot5_home.png') });
  ...
  await page.screenshot({ path: path.join(SHOT_DIR, 'shot5_finale_card.png') });
  await page.evaluate(() => { try { LearnPlayer.close(); } catch (e) {} });
  await page.waitForTimeout(250);

  // ==== 6) Этап 5: карточка рекомендации на результате теста (скриншот) ====
  await page.evaluate(async () => {
  ...
  await page.screenshot({ path: path.join(SHOT_DIR, 'shot5_test_rec.png') });
  await page.evaluate(() => { try { LearnPlayer.close(); } catch (e) {} });
  await page.waitForTimeout(250);

  // ==== 7) Смоки lp5:* в V10.smoke ====
  const sm = await page.evaluate(() => {
    const all = (window.V10 && V10.smoke && V10.smoke.checks) || [];
    const lp5 = all.filter(c => /^lp5:/.test(c.name));
    return { total: all.length, lp5: lp5.map(c => c.name + '=' + (c.ok ? 'ok' : 'FAIL')), allOk: all.every(c => c.ok) };
  });
  ok('7. V10.smoke: lp5:* зелёные, весь смок без FAIL', sm.lp5.length >= 5 && sm.allOk, JSON.stringify(sm));

  // ==== 8) Прогон 20 уроков плеером + консоль ====
  ...
  ok('8. Автопрогон 20 уроков плеером (включая проблемные p0_l6/p1_l1/p3_l5/p6_l2, ps_*, ft*)',
  ...
  ok('9. Консоль: 0 новых pageerror/console.error (унаследованные допущены: ERR_FILE_NOT_FOUND; scrollHeight@renderStep — гонка rAF/close Этапа 1)',
  ...
  console.log(fails === 0 ? 'REGRESSION_5: PASS (' + R.length + ' OK)' : 'REGRESSION_5: ' + fails + ' FAIL');
  await browser.close();
