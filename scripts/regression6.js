// ===== Этап 6, регрессия: старый ридер, Этапы 1–4 не сломаны, смоки lp5:*, скриншоты =====
...
(async () => {
  ...
  await page.screenshot({ path: path.join(SHOT_DIR, 'shot6_reader_dash.png') });
  ...
  await page.screenshot({ path: path.join(SHOT_DIR, 'shot6_home.png') });
  ...
  await page.screenshot({ path: path.join(SHOT_DIR, 'shot6_finale_card.png') });
  await page.evaluate(() => { try { LearnPlayer.close(); } catch (e) {} });
  await page.waitForTimeout(250);
  ...
  await page.screenshot({ path: path.join(SHOT_DIR, 'shot6_test_rec.png') });
  await page.evaluate(() => { try { LearnPlayer.close(); } catch (e) {} });
  await page.waitForTimeout(250);
  ...
  console.log(fails === 0 ? 'REGRESSION_6: PASS (' + R.length + ' OK)' : 'REGRESSION_6: ' + fails + ' FAIL');
  await browser.close();
