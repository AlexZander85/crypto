# tools/build_all_144_quiz_bank.py
import os
import json
import re

ROOT = r'D:\crypto'
BANK_144 = os.path.join(ROOT, 'docs', 'psy_curriculum', 'SELF_CHECK_TESTS_BANK_144.md')

# Load the 48 lessons from core_p8.json
...
# Load existing 120 questions
with open(os.path.join(ROOT, 'tools', 'quiz_psy_120.json'), 'r', encoding='utf-8') as f:
    existing_120 = json.load(f)

# First 24 questions are for base lessons 1-8
all_144_questions = []
all_144_questions.extend(existing_120[:24])

# Next 120 questions are for lessons 9-48 (3 per lesson)
for i, l in enumerate(lessons[8:], start=9):
    ...
    all_144_questions.extend([q1, q2, q3])

print(f"Generated total {len(all_144_questions)} self-check questions for all 48 lessons (144 questions).")

with open(os.path.join(ROOT, 'tools', 'quiz_psy_144.json'), 'w', encoding='utf-8') as f:
    json.dump(all_144_questions, f, ensure_ascii=False, indent=2)
