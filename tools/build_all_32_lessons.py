...
# 1. Load existing lessons 1-8 from core_p8.json
with open(os.path.join(ROOT, 'saas', 'content', 'ru', 'core_p8.json'), 'r', encoding='utf-8') as f:
    core_p8 = json.load(f)
    existing_8 = core_p8['lessons'][:8]

print(f"Loaded {len(existing_8)} existing base psychology lessons (П1-П8)")

# 2. Parse curriculum markdown files for lessons 9-32
module_files = [
...
new_lessons = []
# Map 24 new lessons to П9-П32:
# 1..5 in Mod 1 -> 9..13
# 6..11 in Mod 2 -> 14..19
# 12..17 in Mod 3 -> 20..25
# 18..20, 22 in Mod 4 -> 26..29 (note: 21 pre-mortem is already covered in П7)
# 23..24 in Mod 5 -> 30..31
# 25+32 in Mod 5/6 -> 32
...
lesson_mapping = [
    ...
    (29, 22, "widget_ps_l29_inchworm"),
    (30, 23, "widget_ps_l30_currency_toggle"),
    (31, 24, "widget_ps_l31_habit_breaker"),
    (32, 32, "widget_ps_l32_freedom_calc"),
]

for target_num, src_num, wid_id in lesson_mapping:
    sec_text = parsed_lessons[src_num]
    l_obj = parse_markdown_lesson(sec_text, target_num, f"ps_l{target_num}")
    # Ensure correct interactive block id
    for blk in l_obj['blocks']:
        if blk.get('type') == 'interactive_psy':
            blk['id'] = wid_id
    new_lessons.append(l_obj)

all_32_lessons = existing_8 + new_lessons
print(f"Total compiled lessons: {len(all_32_lessons)} (П1-П32)")

# Split into PSY_LESSONS (П1-П16) and PSY_LESSONS_2 (П17-П32)
psy1_lessons = all_32_lessons[:16]
psy2_lessons = all_32_lessons[16:]

# Generate JS code for the 24 new interactive widgets
widget_js_code = """
/* =========================================================================
...
print("Generated full widget JS code.")

# Save compiled core_p8.json
p8_data = {
    "meta": { "demo": False, "locale": "ru", "phase": 8, "generated": "2026-08-28" },
    "lessons": all_32_lessons
}
with open(os.path.join(ROOT, 'saas', 'content', 'ru', 'core_p8.json'), 'w', encoding='utf-8') as f:
    json.dump(p8_data, f, ensure_ascii=False, indent=2)

print("Saved updated core_p8.json with 32 lessons.")

# Save compiled JSON arrays for embedding in index_v9.html and saas/public/index.html
...
with open(os.path.join(ROOT, 'tools', 'psy_widgets_ext.js'), 'w', encoding='utf-8') as f:
    f.write(widget_js_code)

print("Saved tools/psy_lessons_1.json, tools/psy_lessons_2.json, and tools/psy_widgets_ext.js.")
