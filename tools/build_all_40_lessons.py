...
# 2. Parse all 32 curriculum lessons from MODULE 1-6
module_files = [
...
print(f"Parsed {len(parsed_lessons)} curriculum lessons from Markdown")

# Mapping all 32 curriculum lessons directly to П9–П40
widget_names = [
...
for target_num, src_num, wid_id in widget_names:
    ...
    for blk in l_obj['blocks']:
        if blk.get('type') == 'interactive_psy':
            blk['id'] = wid_id
    new_32_lessons.append(l_obj)

all_40_lessons = base_8 + new_32_lessons
print(f"Total compiled lessons: {len(all_40_lessons)} (П1–П40)")

# Save core_p8.json with all 40 lessons
core_p8_data = {
    "meta": { "demo": False, "locale": "ru", "phase": 8, "generated": "2026-08-28" },
    "lessons": all_40_lessons
}
with open(CORE_P8_PATH, 'w', encoding='utf-8') as f:
    json.dump(core_p8_data, f, ensure_ascii=False, indent=2)

print("Saved core_p8.json with 40 lessons.")

# Split into 2 halves for HTML script loading: П1–П20 and П21–П40
psy1_lessons = all_40_lessons[:20]
psy2_lessons = all_40_lessons[20:]
...
print("Exported psy_lessons_1.json (20) and psy_lessons_2.json (20).")
