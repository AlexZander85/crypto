...
# 2. Parse all 40 curriculum lessons from MODULE 1-7
module_files = [
...
print(f"Parsed {len(parsed_lessons)} curriculum lessons from Markdown (32 + 8 = 40)")

# Mapping all 40 curriculum lessons to П9–П48
widget_names = [
...
for target_num, src_num, wid_id in widget_names:
    ...
    for blk in l_obj['blocks']:
        if blk.get('type') == 'interactive_psy':
            blk['id'] = wid_id
    new_40_lessons.append(l_obj)

all_48_lessons = base_8 + new_40_lessons
print(f"Total compiled lessons: {len(all_48_lessons)} (П1–П48)")

# Save core_p8.json with all 48 lessons
core_p8_data = {
    "meta": { "demo": False, "locale": "ru", "phase": 8, "generated": "2026-08-28" },
    "lessons": all_48_lessons
}
with open(CORE_P8_PATH, 'w', encoding='utf-8') as f:
    json.dump(core_p8_data, f, ensure_ascii=False, indent=2)

print("Saved core_p8.json with 48 lessons.")

# Split into 2 halves for HTML script loading: П1–П24 and П25–П48
psy1_lessons = all_48_lessons[:24]
psy2_lessons = all_48_lessons[24:]
