# tools/inject_all_40_into_both.py
import os
import re
import json
...
with open(os.path.join(ROOT, 'tools', 'quiz_psy_120.json'), 'r', encoding='utf-8') as f:
    quiz_120_json = f.read()
    quiz_120_data = json.loads(quiz_120_json)
...
def update_file(fpath):
    print(f"Updating 40 lessons in {os.path.basename(fpath)}...")
    ...
    if 'const QUIZ_PSY = [' in html:
        ...
        html = html[:q_start] + 'const QUIZ_PSY = ' + quiz_120_json + ';' + html[q_arr_end:]
    else:
        marker = 'function setQuizMode(mode, btn){'
        html = html.replace(marker, 'const QUIZ_PSY = ' + quiz_120_json + ';\n\n' + marker)

    # 4. Update QUIZ button
    if 'id="quiz_mode_psy"' in html:
        html = re.sub(r'<button class="chip" id="quiz_mode_psy"[^>]*>[^<]*</button>',
                      '<button class="chip" id="quiz_mode_psy" onclick="setQuizMode(\'psy\', this)">🧘 Психология и риск-контроль (120 кейсов)</button>',
                      html)

    # 5. Update Phase test and Phase passport
    html = re.sub(r'🧠 Аттестация: [^"\']+', '🧠 Аттестация: Академия психологии и риск-инженерии (120 ситуаций)', html)
    html = re.sub(r'\d+ уроков и \d+ интерактивных тренажёров о главном риске', '40 уроков и 40 интерактивных тренажёров о главном риске', html)
    html = re.sub(r'Пройти \d+ уроков П1–П\d+', 'Пройти 40 уроков П1–П40', html)
    html = re.sub(r'Пройти все \d+ психологических тренажёров', 'Пройти все 40 психологических тренажёров', html)
    html = re.sub(r'Сдать аттестацию по психологии \(\d+ вопрос[^)]*\)', 'Сдать аттестацию по психологии (120 вопросов, порог 80%)', html)

    # 6. Replace widgets ext JS
    marker = '/* ---------- Реестр и встраивание в диспетчер виджетов ---------- */'
    # remove any existing ext widgets block
    ext_start = html.find('/* =========================================================================\n   V5-PSY-EXT:')
    if ext_start != -1:
    ...
    ids_lines = []
    for i in range(0, len(all_40_widget_ids), 4):
        chunk = all_40_widget_ids[i:i+4]
...
phase_8_test = {
    "phase": 8,
    "title": "🧠 Аттестация: Академия психологии и риск-инженерии (120 ситуаций)",
    "threshold": 80,
    "questions": quiz_120_data
}
...
print("Updated tests.json with 120-question Phase 8 test.")
...
all_nums_str = ",".join([f"П{i}" for i in range(1, 41)])
ext_code = re.sub(
...
# Update saas/test/api.test.mjs (demo pack = 20 phase0 + 40 psy = 60 lessons)
...
api_test_code = re.sub(
    ...
    "demoPack.body?.lessons?.length === 60",
    api_test_code
)
