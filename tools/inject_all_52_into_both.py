# tools/inject_all_52_into_both.py
import os
import re
import json
...
with open(os.path.join(ROOT, 'tools', 'quiz_psy_156.json'), 'r', encoding='utf-8') as f:
    quiz_156_json = f.read()
    quiz_156_data = json.loads(quiz_156_json)
...
all_52_widget_ids = [
    ...
    'widget_ps_l49_alpha_paradox','widget_ps_l50_conviction_tiers','widget_ps_l51_narrative_cycle','widget_ps_l52_trade_lifecycle'
]

def update_file(fpath):
    print(f"Updating 52 lessons in {os.path.basename(fpath)}...")
    ...
    if 'const QUIZ_PSY = [' in html:
        ...
        html = html[:q_start] + 'const QUIZ_PSY = ' + quiz_156_json + ';' + html[q_arr_end:]
    else:
        marker = 'function setQuizMode(mode, btn){'
        html = html.replace(marker, 'const QUIZ_PSY = ' + quiz_156_json + ';\n\n' + marker)

    # 4. Update QUIZ button
    if 'id="quiz_mode_psy"' in html:
        html = re.sub(r'<button class="chip" id="quiz_mode_psy"[^>]*>[^<]*</button>',
                      '<button class="chip" id="quiz_mode_psy" onclick="setQuizMode(\'psy\', this)">🧘 Психология и риск-контроль (156 кейсов)</button>',
                      html)

    # 5. Update Phase test and Phase passport
    html = re.sub(r'🧠 Аттестация: [^"\']+', '🧠 Аттестация: Академия психологии и риск-инженерии (156 ситуаций)', html)
    html = re.sub(r'\d+ уроков и \d+ интерактивных тренажёров о главном риске', '52 урока и 52 интерактивных тренажёра о главном риске', html)
    html = re.sub(r'Пройти \d+ уроков П1–П\d+', 'Пройти 52 урока П1–П52', html)
    html = re.sub(r'Пройти все \d+ психологических тренажёров', 'Пройти все 52 психологических тренажёра', html)
    html = re.sub(r'Сдать аттестацию по психологии \(\d+ вопрос[^)]*\)', 'Сдать аттестацию по психологии (156 вопросов, порог 80%)', html)
    ...
    ids_lines = []
    for i in range(0, len(all_52_widget_ids), 4):
        chunk = all_52_widget_ids[i:i+4]
...
phase_8_test = {
    "phase": 8,
    "title": "🧠 Аттестация: Академия психологии и риск-инженерии (156 ситуаций)",
    "threshold": 80,
    "questions": quiz_156_data
}
...
print("Updated tests.json with 156-question Phase 8 test.")
...
ext_code = re.sub(
    ...
    "expect(psy.length === 52, `PSY=${psy.length}, ожидалось 52 (П1–П52)`);",
    ext_code
)

all_nums_str = ",".join([f"П{i}" for i in range(1, 53)])
ext_code = re.sub(
...
# Update saas/test/api.test.mjs (demo pack = 20 phase0 + 52 psy = 72 lessons)
...
api_test_code = re.sub(
    ...
    "demoPack.body?.lessons?.length === 72",
    api_test_code
)
