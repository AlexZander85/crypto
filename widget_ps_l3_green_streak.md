# W‑16 · `widget_ps_l3_green_streak` — «Зелёная полоса»

**Спека (5 строк перед кодом)**
...
1. Повторное открытие урока: интервал дня и rAF убираются через `box._expTimers/_expRaf`, слушатель `resize` — через `box._expResize`; дублей нет.
2. Канон виден на сцене: подпись под лентой дней («+12 % при ×1; −5/−4/−3 %»), карточки разбора (−12 % vs −24 % при ×2, +13.6 % vs +31.6 %), «что если» с формулой DD/(1−DD).
3. «Ага»-сцена: на первом красном дне — вспышка, тег «рынок», вертикальная линия «удар», расхождение кривых; после третьего красного — разбор с «ценой сирены» в рублях.
4. Прогрессия допуска: раунд 1 — только сила воли (кнопка «Вернуть на ×1»); с раунда 2 открывается устав‑блок — шёпот продолжается, но зачёркнут («устав: нет»), ползунок заблокирован. История раундов позволяет сравнить оба режима.
5. Ширина 360 px: сетка HUD `auto-fit`, ряд «шкала + ползунок» переносится, таблица истории — 7 узких колонок моноширинным 12 px без выхода за контейнер (при необходимости уменьшите `padding` ячеек до 2 px).